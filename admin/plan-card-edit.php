    ...
    <div class="col col-6 offset-3">
        ...
        <form action="#" method="post" id="app" enctype="multipart/form-data">
            ...
            <div class="from-group py-3">
                <input type="file" id="files" name="cardimg" />
                <img id="image" />
            </div>
...
<?php
$edit_id = $_GET['edit'];
if (isset($_POST['update']) && !empty($img_name = $_FILES['cardimg']['name'])) {
    $title = $_POST['title'];
...
} elseif (isset($_POST['update']) && empty($img_name = $_FILES['cardimg']['name'])) {
    $title = $_POST['title'];
    $title = mysqli_real_escape_string($con, $title);
    $content = $_POST['content'];
    $content = mysqli_real_escape_string($con, $content);
